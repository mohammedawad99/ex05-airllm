...
| R-BASE-01 | Attempt a **direct baseline** (Ollama or HF `transformers`); document live behavior incl. failure/slowdown | A §5.2 | PARTIALLY_EVIDENCED | results/measurements/transformers_cpu_qwen2_0_5b/summary.csv, docs/MEASUREMENT_RUNS.md | R-NONREPRO | Stage 5B: a **direct HF `transformers` CPU** baseline on Qwen2-0.5B ran 6× successfully (real runtime/throughput/peak-RAM). Full baseline on a larger/selected model still pending; AirLLM is a separate (blocked) path. |
...
| R-QUANT-01 | Apply and compare **quantization** levels (e.g. FP16, Q8, Q4) and document the effect of each | A §5.3 | PARTIALLY_EVIDENCED | results/measurements/transformers_cpu_int8_quantization_qwen2_0_5b/, docs/MEASUREMENT_RUNS.md §9 | R-AIRLLM-COMPAT | Stage 9C Route A: **FP32 vs PyTorch dynamic INT8 measured** (12/12, no download) — INT8 ≈3.6× faster but **output quality degraded**, RAM ≈1.5% lower (honest trade-off). **Dynamic INT8 only — NOT GGUF/Q4/Q8.** Low-bit GGUF Q4/Q8 sweep (Route B) remains NOT_DONE / approval-gated → not fully SATISFIED. |
...
| R-MEAS-QUAL | **Qualitative** assessment of output quality per quantization level | A §5.4 | PARTIALLY_EVIDENCED | results/measurements/transformers_cpu_int8_quantization_qwen2_0_5b/*.json (output_preview/output_text), docs/MEASUREMENT_RUNS.md §9 | — | Stage 9C Route A: per-variant output samples committed — FP32 coherent vs **INT8 degraded/incoherent** (honest quality regression). Covers FP32-vs-dynamic-INT8 only; GGUF Q4/Q8 quality samples pending Route B. |
