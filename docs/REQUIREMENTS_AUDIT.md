...
| R-COST-ONPREM | Compute On-Prem cost: CAPEX amortized + OPEX (electricity, maintenance) per request | A §5.5 | PARTIALLY_EVIDENCED | results/analysis/cost_model_v2.json, figures/final_cost_break_even.png, docs/ANALYSIS.md §9 | R-OVERCLAIM | **Stage 11A cost v2:** **nonzero allocated CAPEX** ($900×25%=$225 effective, amortized 4yr → **$4.6875/month**) + electricity OPEX from measured throughput. CAPEX dominates local cost. Assumption-based (dated 2026-06-21), not verified. |
| R-COST-BREAKEVEN | Find and graph the **break-even point** (requests/tokens) On-Prem vs API | A §5.5 | PARTIALLY_EVIDENCED | results/analysis/cost_model_v2.json, figures/final_cost_break_even.png, docs/ANALYSIS.md §9 | R-OVERCLAIM | **Stage 11A:** meaningful **amortized break-even ≈47k req/month** (gpt-4o-mini) / **≈13k** (gpt-4.1-mini); electricity-only break-even 0. Multi-scenario figure under **dated assumptions (2026-06-21)**, NOT verified market pricing. |
...
| R-CONCEPT-ROOFLINE | Classify each phase as **compute-bound vs memory-bound** (Roofline-style argument) | A §3 | PARTIALLY_EVIDENCED | results/analysis/roofline_classification.json, figures/final_roofline_classification.png, docs/ANALYSIS.md §9 | R-NONREPRO | **Stage 11A:** a **Roofline-style qualitative classification** of all five evidence groups (memory/CPU-bound, prefill/decode, memory-capacity bound at 7B), anchored to measured throughput/TTFT. Explicitly **not** a formal hardware roofline benchmark. |
| R-EXT-01 | Deliver **≥1 original extension** (bottleneck-shift map / quant Pareto frontier / AirLLM decision matrix / break-even simulator / LoRA-QLoRA / multi-model compare) | A §5.7 | PARTIALLY_EVIDENCED | src/ex05_airllm/cost_model.py, results/analysis/cost_model_v2.json, docs/ANALYSIS.md §9 | — | **Stage 11A:** a parameterized **CAPEX-aware break-even cost model v2** (multi-scenario, dated-assumption) delivered, plus the AirLLM forensic failure analysis. |
