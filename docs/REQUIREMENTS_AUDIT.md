...
| R-MEAS-TTFT | Measure **TTFT** (Time To First Token) — prefill + KV-cache build cost | A §5.4 | PLANNED | results/measurements/transformers_cpu_qwen2_0_5b/summary.csv, docs/MEASUREMENT_RUNS.md | R-NONREPRO | Stage 5B: recorded **None** (HF `generate()` not token-streamed → no first-token hook). A streaming hook is needed to evidence TTFT; documented honestly, not faked. |
| R-MEAS-TPOT | Measure **TPOT / ITL** (per-output-token latency / inter-token latency) | A §5.4 | PARTIALLY_EVIDENCED | results/measurements/transformers_cpu_qwen2_0_5b/summary.csv, docs/MEASUREMENT_RUNS.md | R-NONREPRO | Stage 5B: **approximate** TPOT = generation_seconds/output_tokens (no TTFT to subtract prefill); caveat in each record's `notes`. |
...
| R-CONFIG-ARCH | Config hierarchy: versioned `config/*` , `.env` (ignored), `.env-example` (committed) | G §7.3 | PARTIALLY_EVIDENCED | config/, .env-example | R-SECRETS | Stage 2A: `config/experiment.example.toml` (versioned). `.env-example` added when a token is first needed. |
