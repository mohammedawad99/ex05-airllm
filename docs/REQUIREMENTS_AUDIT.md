...
| R-AIR-01 | Run the same task with **AirLLM** (layer-wise loading) | A §5.3 | PLANNED | results/airllm/*.json, reports/airllm.md | R-AIRLLM-SHARD, R-AIRLLM-TIED, R-AIRLLM-META | 3A format fail → 3B re-shard+untie fixed format (AirLLM loads & starts) → 3B/3C CPU forward **fails** with meta-device error; **3C ruled out torch** (2.4.1 & 2.12.1 identical). Still **PLANNED / not evidenced** (no generation). Next = HF CPU fallback (ADR-0016). See `SMOKE_RUN.md` §6–§7. |
...
| R-REPRO | All runs reproducible: documented assumptions, seeds, commands, environment; numbers regenerate from scripts | A §5.5, A §10, G §9 | PARTIALLY_EVIDENCED | results/stage3d_smoke_transformers_qwen2_0_5b_cpu.json, src/ex05_airllm/smoke_transformers_cpu.py | R-NONREPRO | Stage 3D: a tiny offline HF CPU smoke writes a **schema-valid result JSON** from a documented command (timers/RSS/tokens) → the result-writing pipeline is proven. Full metric suite + reproduction section still to come. |
