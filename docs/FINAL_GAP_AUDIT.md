...
| 8 | Cost analysis (API / On-Prem / break-even) | PARTIALLY_SATISFIED | `results/analysis/cost_model_v2.json`, `figures/final_cost_break_even.png`, `docs/ANALYSIS.md` §9 | **Stage 11A cost model v2**: nonzero **allocated CAPEX** ($225 → $4.6875/month) → meaningful amortized **break-even ≈47k req/month (gpt-4o-mini) / ≈13k (gpt-4.1-mini)**; electricity-only break-even 0; CAPEX dominates local cost. All prices/tariff/FX are **dated assumptions (2026-06-21)**, not guaranteed pricing. (v1 assumption estimate retained.) |
| 9 | Graphs / figures | SATISFIED | `figures/*.png`, `docs/ANALYSIS.md` §4 / §9 | 4 original figures (runtime, throughput, peak RAM, v1 break-even) **plus 4 Stage 11A figures** (`final_quantization_speed_ram`, `final_ttft_tpot`, `final_cost_break_even`, `final_roofline_classification`) — all plain matplotlib, default colors, no subplots, from committed data. |
| 10 | Lecture concept linkage | PARTIALLY_SATISFIED | `README.md` §9, `reports/final_report.md` §5–6, `results/analysis/roofline_classification.json` | Concepts tied to *our* evidence with measured-vs-discussed markers; Stage 11A adds a **Roofline-style qualitative classification** (+figure) of each stage's memory/compute regime from measured evidence (explicitly **not** a formal hardware roofline). |
| 11 | AI workflow documentation | SATISFIED | `docs/AI_WORKFLOW.md`, `docs/PROMPTS.md` | Prompt-engineering log maintained per stage. |
| 12 | Quality gates / tests | SATISFIED | `tests/`, `pyproject.toml`, `docs/QUALITY.md` | **123 tests, ~96% coverage** (≥85; up from ~88% — Stage 11A added runner + pipeline + cost-v2 tests; both quantization runners now 100% line-covered), ruff check + format clean, every file ≤150 code lines. |
