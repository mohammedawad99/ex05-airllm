...
### Added in Stage 3A (tiny AirLLM smoke)

| id | risk | L | I | trigger / signal | mitigation (preventive) | contingency (if it happens) | owner |
| --- | --- | --- | --- | --- | --- | --- | --- |
| R-AIRLLM-SHARD | **AirLLM needs pre-sharded safetensors + index** — `split_and_save_layers` asserts on `model.safetensors.index.json`; single-file models (e.g. Qwen2-0.5B) lack it and fail to load | **Resolved (3B)** | Med | 3A: `AssertionError: model.safetensors.index.json should exist.` | Re-save the model sharded (`save_pretrained(max_shard_size=...)`); main Qwen2-7B is natively multi-shard | Pre-shard locally (ADR-0014); fixed in 3B re-shard | AI/Human |
| R-AIRLLM-TIED | **AirLLM mishandles tied embeddings** — expects a separate `lm_head.weight`; tied models (Qwen2-0.5B) have none → `IndexError` in `split_and_save_layers` | **Resolved (3B)** | Med | 3B: `IndexError: list index out of range` (`utils.py:302`) | Untie embeddings before saving (clone embed weight → `lm_head.weight`) | Prep script unties (numerically identical); Qwen2-7B is already untied | AI/Human |
| R-AIRLLM-META | **AirLLM CPU runtime meta-device mismatch** — AirLLM builds the model on `meta` and streams decoder layers; non-streamed buffers stay on `meta`. For Qwen2 the **top-level `rotary_emb` (`inv_freq`)** is never moved → `RuntimeError: Tensor on device cpu is not on the expected device meta!` in the forward pass. **Torch version ruled out** (3C: identical failure on torch 2.4.1 *and* 2.12.1). | **Confirmed (3B/3C)** | **High** | 3B/3C: meta/cpu mismatch in `_refs.mul`; AirLLM only relocates rotary for GLM, not Qwen2 | Use a direct HF `transformers` CPU run for the pipeline proof (Stage 3D); AirLLM CPU needs a buffer-materialization patch (fragile) | Stage 3D HF fallback proves the pipeline; AirLLM CPU stays a documented limitation; **fix before any 7B attempt** (same rotary path) | Human/AI |
...
| 2026-06-20 | 3A | Tiny AirLLM smoke on Qwen2-0.5B **failed** at sharding (`model.safetensors.index.json` missing) → added **R-AIRLLM-SHARD** (confirmed). Single-file tiny models need re-sharding; the main Qwen2-7B is natively sharded. Evidence in `SMOKE_RUN.md`; corrective = ADR-0014. |
| 2026-06-20 | 3B | Local re-shard + untie **resolved** R-AIRLLM-SHARD & added/resolved **R-AIRLLM-TIED**; AirLLM then loaded & started inference but **failed** with a CPU meta-device error → added **R-AIRLLM-META** (high). Smoke NOT successful; corrective = pin older torch (ADR-0015) / HF-baseline fallback. Evidence in `SMOKE_RUN.md` §6. |
| 2026-06-20 | 3C | Pinned `torch==2.4.1+cpu` and retested: **identical** meta-device error → **torch ruled out**. Root cause = AirLLM leaves Qwen2's top-level `rotary_emb` on `meta`. Kept torch 2.4.1 pin; recommend **Stage 3D HF CPU fallback** (ADR-0015 updated, ADR-0016). Evidence in `SMOKE_RUN.md` §7. |
| 2026-06-20 | 3D | **Transformers CPU fallback smoke SUCCEEDED** (offline, cache-only Qwen2-0.5B) → measurement pipeline proven (R-REPRO partially evidenced). AirLLM CPU **remains blocked** (R-AIRLLM-META, not evidenced). Not a benchmark. Evidence in `SMOKE_RUN.md` §8. |
